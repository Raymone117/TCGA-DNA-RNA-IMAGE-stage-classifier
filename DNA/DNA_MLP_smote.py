...
# ------------------ config ------------------
SEED = 42
USE_SMOTE = True             # Whether to oversample minority classes
EPOCHS = 50
BATCH_SIZE = 64
LR = 1e-3
...
# 4) (Optional) SMOTE to oversample Stage I & IV
if USE_SMOTE:
    counts = np.bincount(y_train)
    stageIII_count = counts[2]   # Stage III count
    sampling_strategy = {0: stageIII_count, 3: stageIII_count}
    sm = SMOTE(random_state=SEED, sampling_strategy=sampling_strategy, k_neighbors=2)
...
plt.title("Confusion Matrix - MLP (1000 features, 70/30 split)")
plt.show()
