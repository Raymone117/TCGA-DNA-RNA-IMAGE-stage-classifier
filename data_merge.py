...
CNV_DIR = "TCGA_BRCA_CNV"           # Folder containing all CNV files
METADATA_PATH = "metadata.json"     # Metadata file downloaded from GDC
CLINICAL_PATH = "clinical.tsv"      # Clinical data file
OUTPUT_PATH = "BRCA_CNV_stage.csv"  # Final merged dataset
...
print(f"clinical.tsv processed, {len(clinical_df)} patients have valid stage labels")
...
for file_name, patient_id in tqdm(file_to_patient.items()):
    ...
    cnv_dict = dict(zip(df["gene_name"], df["copy_number"]))
    cnv_data[patient_id] = cnv_dict

print(f"Collected CNV data for {len(cnv_data)} patients")

# Step 4: Convert to DataFrame and merge with clinical
...
# Step 7: Visualization (Stage Distribution)
